...
def main():
    ...
    # Training loop (using test data for training)
    for epoch in range(epochs):
        loss_epoch = 0
        for i in range(0, len(x_test), batch_size):  # Change here: using x_test, y_test for training
            # Handle case where the last batch may be smaller than batch_size
            batch_x = np.array(x_test[i:i+batch_size]).reshape(-1, 784)  # Reshape to (batch_size, 784)
            batch_y = labels_encode(y_test[i:i+batch_size], 10)  # Using y_test as labels for training
            loss = nn.backpropagate(batch_x, batch_y, learning_rate)
            loss_epoch += loss
        
        print(f"Epoch {epoch+1}/{epochs}, Loss: {loss_epoch / (len(x_test) / batch_size):.4f}")

    # Save the model after training
    save_model(nn, "model.npz")

    # Select a random image from the training data for testing
    index = random.randint(0, len(x_train) - 1)  # Random index from the original training data
    plt.imshow(x_train[index], cmap=plt.cm.gray)  # Visualizing the test data from the original training set
    plt.title(f"Actual Label: {y_train[index]}")
    plt.show()

    # Predict using the trained model
    sample_input = np.array(x_train[index]).reshape(1, 784)  # Reshape test data to match input
